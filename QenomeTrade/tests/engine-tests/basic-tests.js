...
async function temp() {
    ...
    await api.tx.qenome.registerNewOrderbook(2, 1).signAndSend(alice, {nonce: 1});

    // Let's place some orders and check if we got the expected results
    // Alice places buy limit orders
    await api.tx.qenome.submitOrder("BidLimit", tradingPairID, 820 * UNIT, 0.2 * UNIT).signAndSend(alice, {nonce: 2});
    await api.tx.qenome.submitOrder("BidLimit", tradingPairID, 800 * UNIT, 0.1 * UNIT).signAndSend(alice, {nonce: 3});
    await api.tx.qenome.submitOrder("BidLimit", tradingPairID, 850 * UNIT, 0.1 * UNIT).signAndSend(alice, {nonce: 4});
    await api.tx.qenome.submitOrder("BidLimit", tradingPairID, 840 * UNIT, 0.1 * UNIT).signAndSend(alice, {nonce: 5});
    await api.tx.qenome.submitOrder("BidLimit", tradingPairID, 900 * UNIT, 0.1 * UNIT).signAndSend(alice, {nonce: 6});
    // Bob places sell limit orders
    await api.tx.qenome.submitOrder("AskLimit", tradingPairID, 1075 * UNIT, 0.2 * UNIT).signAndSend(bob, {nonce: 1});
    await api.tx.qenome.submitOrder("AskLimit", tradingPairID, 1100 * UNIT, 0.1 * UNIT).signAndSend(bob, {nonce: 2});
    await api.tx.qenome.submitOrder("AskLimit", tradingPairID, 1060 * UNIT, 0.1 * UNIT).signAndSend(bob, {nonce: 3});
    await api.tx.qenome.submitOrder("AskLimit", tradingPairID, 1040 * UNIT, 0.1 * UNIT).signAndSend(bob, {nonce: 4});
    await api.tx.qenome.submitOrder("AskLimit", tradingPairID, 1000 * UNIT, 0.1 * UNIT).signAndSend(bob, {nonce: 5});

    // Let's place some Market Orders
    await api.tx.qenome.submitOrder("BidMarket", tradingPairID, 0.05 * UNIT, 0).signAndSend(alice, {nonce: 7});
    await api.tx.qenome.submitOrder("AskMarket", tradingPairID, 0, 0.005 * UNIT).signAndSend(bob, {nonce: 6});
    await api.tx.qenome.submitOrder("BidMarket", tradingPairID, 0.016 * UNIT, 0).signAndSend(alice, {nonce: 8});
    await api.tx.qenome.submitOrder("AskMarket", tradingPairID, 0, 0.016 * UNIT).signAndSend(bob, {nonce: 7});
